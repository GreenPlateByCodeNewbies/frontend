  ...
  return (
    <div className="p-8 pb-32 relative z-10">
      <div className="flex flex-col items-center mb-12">
        <div className="relative mb-8">
          <div className="w-36 h-36 rounded-[3.5rem] border-[10px] border-white shadow-[0_30px_60px_rgba(0,0,0,0.12)] overflow-hidden bg-gray-50">
            <img src={isUser ? "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=300" : "https://images.unsplash.com/photo-1544644181-1484b3fdfc62?auto=format&fit=crop&q=80&w=300"} className="w-full h-full object-cover" alt="Profile" />
          </div>
          <motion.div initial={{ scale: 0, rotate: -45 }} animate={{ scale: 1, rotate: 0 }} className="absolute -bottom-3 -right-3 bg-green-600 text-white p-4 rounded-3xl shadow-2xl border-[6px] border-white">
            <Award size={24} />
          </motion.div>
        </div>
        <h2 className="text-4xl font-black text-gray-900 tracking-tighter text-center leading-none">{isUser ? 'Alex Rivera' : 'Campus Admin'}</h2>
        <p className="text-gray-500 font-bold uppercase tracking-[0.3em] text-[10px] mt-3">{isUser ? 'Impact Hero' : 'Sustainability Lead'}</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <button onClick={resetApp} className="w-full mt-10 flex items-center justify-between p-7 bg-red-50/60 text-red-600 rounded-[2.5rem] border border-red-100/30 active:scale-95 transition-transform">
          <div className="flex items-center gap-5"><LogOut size={24} /><span className="font-black text-xl tracking-tighter">Sign Out</span></div>
          <ChevronRight size={22} />
        </button>
      </div>
      
      <div className="mt-12 text-center text-gray-300">
        <div className="inline-flex items-center gap-2"><Leaf size={14} /><span className="text-[9px] font-black uppercase tracking-[0.4em]">GreenPlate v2.1.0</span></div>
      </div>
    </div>
  );
};
